...
git config --global alias.wta "worktree add --detach"
...
git config --global alias.tr "! f() { [ ! -z \"\$1\" ] && git branch --track \$1 \$(git remote)/\$1 ; }; f"
...
git config --global alias.rd  "! rd()  { git range-diff \$(git merge-base  \"\$1\" \"\$2\") \"\$1\" \"\$2\" ; }; rd"
git config --global alias.rdu "! rdu() { git range-diff \$(git merge-base  \"@{u}\" \"@\")  \"@{u}\" \"@\"  ; }; rdu"
...
git config --global alias.cop "! checkout_and_pull() { GIT_LFS_SKIP_SMUDGE=1 git co \"\$@\" && git pl && { git lfs pull 2>/dev/null; true; }; }; checkout_and_pull"
...
git config --global alias.swa "! f() { { git for-each-ref --sort=-committerdate --format='%(refname:short)' refs/heads/ ; git for-each-ref --sort=-committerdate --format='%(refname:lstrip=3)' refs/remotes/ ; } | awk '\$0 != \"HEAD\" && seen[\$0]++ == 0' | (command -v fzf >/dev/null 2>&1 && fzf --preview 'git show --stat --color=always {-1}' --bind 'enter:become(git switch {-1})' --height 40% --layout reverse || cat) ; }; f"
